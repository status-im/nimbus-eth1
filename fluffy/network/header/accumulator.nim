...
import
  ...
  ssz_serialization, ssz_serialization/proofs,
  ./header_content,
  ../../common/common_types,
  ../../populate_db

export header_content, kvstore_sqlite3
    ...
    if accumulator.currentEpoch.len() == epochSize:
      ...
      let key = ContentKey(
        ...
        epochAccumulatorKey: EpochAccumulatorKey(
          epochAccumulatorRootHash: rootHash.data))
...
proc buildProof*(db: AccumulatorDB, header: BlockHeader):
  ...
  let
    ...
    epochAccumulatorHash = accumulator.historicalEpochs[epochIndex]

    key = ContentKey(
      contentType: epochAccumulator,
      epochAccumulatorKey: EpochAccumulatorKey(
        epochAccumulatorRootHash: epochAccumulatorHash))

    serializedEpochAccumulator = db.get(key.toContentId())

  if serializedEpochAccumulator.isNone():
    return err("Epoch accumulator not found in database")

  let
    epochAccumulator =
      try:
        SSZ.decode(serializedEpochAccumulator.get(), EpochAccumulator)
      except SszError:
        raiseAssert("Stored data should always deserialize properly")
...
func verifyProof*(
  ...
  let
    epochIndex = getEpochIndex(header)
    epochAccumulatorHash = a.historicalEpochs[epochIndex]

  var rootHash: Digest
  rootHash.data = epochAccumulatorHash
  ...
  verify_merkle_multiproof(@[leave], proof, @[gIndex], rootHash)
