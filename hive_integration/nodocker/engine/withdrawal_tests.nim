...
let wdTestList* = [
  ...
  TestDesc(
    ...
    about:
      "Tests a 8 block re-org using NewPayload. Re-org does not change withdrawals fork height",
  ...
  TestDesc(
    ...
    spec: BlockValueSpec(forkHeight: 1, wdBlockCount: 1),
  ),
  # Base tests
  TestDesc(
  ...
  TestDesc(
    ...
    run: specExecute[WDBaseSpec],
    spec: WDBaseSpec(forkHeight: 1, wdBlockCount: 1, wdPerBlock: 0),
  ),
  TestDesc(
    name: "Corrupted Block Hash Payload (INVALID)",
    about: "Send a valid payload with a corrupted hash using engine_newPayloadV2.",
    run: specExecute[WDBaseSpec],
    spec: WDBaseSpec(forkHeight: 1, wdBlockCount: 1, testCorrupedHashPayloads: true),
  ),
]
