...
import
  eth/common, chronicles, stew/byteutils, ./engine_spec, ../cancun/customizer, ../helper

type SidechainReOrgTest* = ref object of EngineSpec
...
method execute(cs: SidechainReOrgTest, env: TestEnv): bool =
  ...
  let tc = BaseTx(
    recipient: Opt.some(prevRandaoContractAddr), txType: cs.txType, gasLimit: 75000
  )
  ...
  let pbRes = env.clMock.produceSingleBlock(
    BlockProcessCallbacks(
      onNewPayloadBroadcast: proc(): bool =
        ...
        testCond checkPrevRandaoValue(
          env.engine.client, alternativePrevRandao,
          alternativePayload.blockNumber.uint64,
        )
        return true
    )
  )
  ...
  testCond checkPrevRandaoValue(
    env.engine.client, env.clMock.prevRandaoHistory[latestBlockNum], latestBlockNum
  )
  return true

# Test performing a re-org that involves removing or modifying a transaction
type TransactionReOrgScenario = enum
...
method execute(cs: TransactionReOrgTest, env: TestEnv): bool =
  ...
  shadow.sendTransaction = proc(i: int): PooledTransaction {.gcsafe.} =
    let sstoreContractAddr =
      hexToByteArray[20]("0000000000000000000000000000000000000317")
    ...
    let pbRes = env.clMock.produceSingleBlock(
      BlockProcessCallbacks(
        ...
        onGetpayload: proc(): bool =
          ...
          if cs.scenario in [
            TransactionReOrgScenarioReOrgDifferentBlock,
            TransactionReOrgScenarioNewPayloadOnRevert,
          ]:
            # Create side payload with different hash
            let customizer = CustomPayloadData(extraData: Opt.some(@[0x01.byte]))
            shadow.payload =
              customizer.customizePayload(env.clMock.latestExecutableData).basePayload

            testCond shadow.payload.parentHash ==
              env.clMock.latestPayloadBuilt.parentHash:
            ...
            if i == 0:
              ...
              var version = env.engine.version(env.clMock.latestHeader.timestamp)
              let f = env.engine.client.forkchoiceUpdated(
                version, forkchoiceUpdated, Opt.some(payloadAttributes)
              )
        ...
        onForkchoiceBroadcast: proc(): bool =
          if cs.scenario != TransactionReOrgScenarioReOrgBackIn:
            ...
            testCond env.clMock.broadcastForkchoiceUpdated(
              Version.V1, env.clMock.latestForkchoice
            )
    ...
    BlockProcessCallbacks(
      ...
      onGetpayload: proc(): bool =
        ...
        let customizer = CustomPayloadData(
          parentHash: Opt.some(ethHash altParentHash), extraData: Opt.some(@[0x01.byte])
        )
...
type ReOrgPrevValidatedPayloadOnSideChainTest* = ref object of EngineSpec

method withMainFork(
    cs: ReOrgPrevValidatedPayloadOnSideChainTest, fork: EngineFork
): BaseSpec =
  var res = cs.clone()
  res.mainFork = fork
  return res
  ...
  var pbRes = env.clMock.produceBlocks(
    ...
    BlockProcessCallbacks(
      ...
      onGetpayload: proc(): bool =
        ...
        var customData = CustomPayloadData(extraData: Opt.some(toSeq("side")))
  ...
  pbRes = env.clMock.produceSingleBlock(
    BlockProcessCallbacks(
      onGetpayload: proc(): bool =
        ...
        let newPayloadAttributes =
          payloadAttributesCustomizer.getPayloadAttributes(reOrgPayloadAttributes)
        let fcu = ForkchoiceStateV1(
        ...
        let r = env.engine.client.forkchoiceUpdated(
          version, fcu, Opt.some(newPayloadAttributes)
        )
...
type SafeReOrgToSideChainTest* = ref object of EngineSpec
