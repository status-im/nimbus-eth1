...
import
  chronicles, ./engine_spec, ../helper, ../cancun/customizer, ../../../../nimbus/common
...
method execute(cs: InvalidPayloadTestCase, env: TestEnv): bool =
  ...
  if cs.syncing:
    ...
    testCond env.clMock.produceSingleBlock(
      BlockProcessCallbacks(onPayloadProducerSelected: txFunc)
    )
  ...
  var pbRes = env.clMock.produceSingleBlock(
    BlockProcessCallbacks(
      ...
      onGetPayload: proc(): bool =
        ...
        if not cs.emptyTransactions and
            env.clMock.latestPayloadBuilt.transactions.len == 0:
        ...
        if cs.syncing or cs.invalidField == InvalidParentHash:
          ...
          if shadow.invalidDetectedOnSync:
            ...
            r.expectStatusEither(
              [
                PayloadExecutionStatus.invalid, PayloadExecutionStatus.accepted,
        ...
        if not cs.syncing:
          ...
          let p =
            env.engine.client.newPayload(version, env.clMock.latestExecutedPayload)
          ...
          if cs.invalidField == InvalidParentHash:
            ...
            q.expectStatusEither(
              [PayloadExecutionStatus.invalid, PayloadExecutionStatus.syncing]
            )
          ...
          if cs.invalidField != InvalidParentHash:
            ...
            s.expectStatusEither(
              [PayloadExecutionStatus.syncing, PayloadExecutionStatus.invalid]
            )
...
type PayloadBuildAfterInvalidPayloadTest* = ref object of EngineSpec
  invalidField*: InvalidPayloadBlockField

method withMainFork(
    cs: PayloadBuildAfterInvalidPayloadTest, fork: EngineFork
): BaseSpec =
  var res = cs.clone()
  res.mainFork = fork
  return res
      ...
      onPayloadAttributesGenerated: proc(): bool =
        ...
        block:
          ...
          let
            ...
            r = invalidPayloadProducer.client.forkchoiceUpdated(
              version, env.clMock.latestForkchoice, Opt.some(payloadAttributes)
            )
  ...
  let pbRes = env.clMock.produceSingleBlock(
    BlockProcessCallbacks(
      ...
      onPayloadAttributesGenerated: proc(): bool =
        ...
        let txCustomizerData =
          CustomTransactionData(chainID: Opt.some((chainId.uint64 + 1'u64).ChainId))

        shadow.invalidTx = tx
        shadow.invalidTx.tx =
          env.customizeTransaction(sender, shadow.invalidTx.tx, txCustomizerData)
