...
type ForkchoiceStateField* = enum
  HeadBlockHash = "Head"
  SafeBlockHash = "Safe"
  ...
  let pbRes = env.clMock.produceBlocks(
    ...
    BlockProcessCallbacks(
      onGetPayload: proc(): bool =
        ...
        var customData = CustomPayloadData(extraData: Opt.some(@[0x01.byte]))
...
type ForkchoiceUpdatedUnknownBlockHashTest* = ref object of EngineSpec
  field*: ForkchoiceStateField

method withMainFork(
    cs: ForkchoiceUpdatedUnknownBlockHashTest, fork: EngineFork
): BaseSpec =
  var res = cs.clone()
  res.mainFork = fork
  return res
      ...
      BlockProcessCallbacks(
        ...
        onNewPayloadBroadcast: proc(): bool =
          ...
          r = env.engine.client.forkchoiceUpdated(
            version, fcu, Opt.some(payloadAttributes)
          )
          r.expectError()
          return true
      )
