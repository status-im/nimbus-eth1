...
proc txInPayload*(payload: ExecutionPayload, txHash: common.Hash32): bool =
  for txBytes in payload.transactions:
    let currTx = rlp.decode(common.Blob txBytes, Transaction)
    if rlpHash(currTx) == txHash:
      return true

proc checkPrevRandaoValue*(client: RpcClient, expectedPrevRandao: common.Hash32, blockNumber: uint64): bool =
