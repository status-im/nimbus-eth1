...
proc getSidechainforkHeight(ws: ReorgSpec): int =
  if ws.getSidechainBlockTimeIncrements() != ws.getBlockTimeIncrements():
    ...
    if ws.getSidechainSplitHeight() <= ws.forkHeight:
      ...
      let sidechainSplitBlocktimestamp =
        (ws.getSidechainSplitHeight() - 1) * ws.getBlockTimeIncrements()
      let remainingTime =
        ws.getWithdrawalsGenesisTimeDelta() - sidechainSplitBlocktimestamp
      if remainingTime == 0:
        return ws.getSidechainSplitHeight()

      return
        ((remainingTime - 1) div ws.sidechainTimeIncrements) +
        ws.getSidechainSplitHeight()
...
proc execute*(ws: ReorgSpec, env: TestEnv): bool =
  ...
  var
    canonical = Canonical(startAccount: u256(0x1000), nextIndex: 0)
    sidechain = Sidechain(
    ...
    BlockProcessCallbacks(
      onPayloadProducerSelected: proc(): bool =
        ...
        if env.clMock.currentPayloadNumber >= ws.forkHeight.uint64:
          ...
          let wfb =
            ws.generateWithdrawalsForBlock(canonical.nextIndex, canonical.startAccount)
        ...
        if env.clMock.currentPayloadNumber >= ws.getSidechainSplitHeight().uint64:
          ...
          if env.clMock.currentPayloadNumber >= ws.getSidechainforkHeight().uint64:
            ...
            let wfb = ws.generateWithdrawalsForBlock(
              sidechain.nextIndex, sidechain.startAccount
            )
      ...
      onRequestNextPayload: proc(): bool =
        ...
        if env.clMock.currentPayloadNumber >= ws.getSidechainSplitHeight().uint64:
          ...
          var fcState =
            ForkchoiceStateV1(headBlockHash: env.clMock.latestForkchoice.headBlockHash)

          if env.clMock.currentPayloadNumber > ws.getSidechainSplitHeight().uint64:
            let lastSidePayload =
              sidechain.sidechain[env.clMock.currentPayloadNumber - 1]
          ...
          var attr = PayloadAttributes(
            ...
            suggestedFeeRecipient:
              env.clMock.latestPayloadAttributes.suggestedFeeRecipient,
          )

          if env.clMock.currentPayloadNumber > ws.getSidechainSplitHeight().uint64:
            attr.timestamp = w3Qty(
              sidechain.sidechain[env.clMock.currentPayloadNumber - 1].timestamp,
              ws.getSidechainBlockTimeIncrements(),
            )
          elif env.clMock.currentPayloadNumber == ws.getSidechainSplitHeight().uint64:
            attr.timestamp = w3Qty(
              env.clMock.latestHeader.timestamp, ws.getSidechainBlockTimeIncrements()
            )
          else:
          ...
          info "Requesting sidechain payload", number = env.clMock.currentPayloadNumber
      ...
      onGetPayload: proc(): bool =
        ...
        if env.clMock.latestPayloadBuilt.blockNumber.uint64 >=
            ws.getSidechainSplitHeight().uint64:
  ...
  if ws.forkHeight < ws.getSidechainforkHeight():
    ...
    for i in 0 ..< height:
      let wfb =
        ws.generateWithdrawalsForBlock(sidechain.nextIndex, sidechain.startAccount)
      ...
      let
        attr = PayloadAttributes(
          timestamp: w3Qty(
            sidechain.sidechain[sidechain.height].timestamp,
            ws.getSidechainBlockTimeIncrements(),
          ),
          prevRandao: env.clMock.latestPayloadAttributes.prevRandao,
          suggestedFeeRecipient:
  ...
  let res =
    ws.wdHistory.verifyWithdrawals(sidechain.height, Opt.none(uint64), env.client)
  testCond res.isOk
  ...
  let r2 = sidechain.wdHistory.verifyWithdrawals(
    sidechain.height, Opt.none(uint64), env.client
  )
  testCond r2.isOk
  ...
  # and the fork.
  # We check on `latest`.
  let r3 = ws.wdHistory.verifyWithdrawals(
    uint64(ws.forkHeight - 1), Opt.none(uint64), env.client
  )
  testCond r3.isOk

  # Re-Org back to the canonical chain
  let fcState =
    ForkchoiceStateV1(headBlockHash: env.clMock.latestPayloadBuilt.blockHash)
