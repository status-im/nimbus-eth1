...
type
  WDBaseSpec* = ref object of BaseSpec
    ...
    wdHistory*: WDHistory
      # Internal withdrawals history that keeps track of all withdrawals
...
const
  WARM_COINBASE_ADDRESS =
    hexToByteArray[20]("0x0101010101010101010101010101010101010101")
  PUSH0_ADDRESS = hexToByteArray[20]("0x0202020202020202020202020202020202020202")
  MAINNET_MAX_WITHDRAWAL_COUNT_PER_BLOCK* = 16
  TX_CONTRACT_ADDRESSES = [WARM_COINBASE_ADDRESS, PUSH0_ADDRESS]
...
func getGenesis*(ws: WDBaseSpec, param: NetworkParams) =
  ...
  param.genesis.alloc[WARM_COINBASE_ADDRESS] =
    GenesisAccount(code: @warmCoinbaseCode, balance: 0.u256)
  ...
  param.genesis.alloc[PUSH0_ADDRESS] = GenesisAccount(code: @push0Code, balance: 0.u256)
...
proc verifyContractsStorage(ws: WDBaseSpec, env: TestEnv): Result[void, string] =
  ...
  let
    ...
    r = env.client.storageAt(
      WARM_COINBASE_ADDRESS, latestPayloadNumber.u256, latestPayloadNumber
    )
    p = env.client.storageAt(PUSH0_ADDRESS, 0.u256, latestPayloadNumber)

  if latestPayloadNumber >= ws.forkHeight.uint64:
    # Shanghai
    r.expectStorageEqual(WARM_COINBASE_ADDRESS, 100.u256.w3FixedBytes)
      # WARM_STORAGE_READ_COST
    p.expectStorageEqual(PUSH0_ADDRESS, latestPayloadNumber.u256.w3FixedBytes)
      # tx succeeded
  else:
    # Pre-Shanghai
    r.expectStorageEqual(WARM_COINBASE_ADDRESS, 2600.u256.w3FixedBytes)
...
func generateWithdrawalsForBlock*(
    ws: WDBaseSpec, nextIndex: int, startAccount: UInt256
): WithdrawalsForBlock =
  let differentAccounts = ws.getWithdrawableAccountCount()
...
proc execute*(ws: WDBaseSpec, env: TestEnv): bool =
  ...
  var pbRes = env.clMock.produceBlocks(
    ws.getPreWithdrawalsBlockCount,
      ...
      onPayloadProducerSelected: proc(): bool =
        ...
        if not ws.skipBaseVerifications:
          ...
          var r = env.client.forkchoiceUpdatedV2(
            ForkchoiceStateV1(headBlockHash: w3Hash env.clMock.latestHeader),
            Opt.some(
              PayloadAttributes(
                timestamp:
                  w3Qty(env.clMock.latestHeader.timestamp, ws.getBlockTimeIncrements()),
          ...
          let expectationDescription =
            "Sent pre-shanghai Forkchoice using ForkchoiceUpdatedV2 + Withdrawals, error is expected"
          ...
          let expectationDescription2 =
            "Sent pre-shanghai Forkchoice ForkchoiceUpdatedV2 + null withdrawals, no error is expected"
      ...
      onGetPayload: proc(): bool =
        if not ws.skipBaseVerifications:
          ...
          let customizer = CustomPayloadData(
            ...
            parentBeaconRoot:
              ethHash env.clMock.latestPayloadAttributes.parentBeaconBlockRoot,
          )
          let payloadPlusWithdrawals =
            customizer.customizePayload(env.clMock.latestExecutableData).basePayload
  ...
  pbRes = env.clMock.produceBlocks(
    ws.wdBlockCount,
    BlockProcessCallbacks(
      onPayloadProducerSelected: proc(): bool =
        if not ws.skipBaseVerifications:
          ...
          let expectationDescription =
            "Sent shanghai fcu using PayloadAttributesV1, error is expected"
          r.expectErrorCode(engineApiInvalidParams, expectationDescription)
        ...
        if not ws.skipBaseVerifications:
          ...
          testCond sentList.len == wdList.len:
            error "Incorrect list of withdrawals on built payload",
              want = sentList.len, get = wdList.len
      ...
      onNewPayloadBroadcast: proc(): bool =
        ...
        if not ws.skipBaseVerifications:
          let addrList = ws.wdHistory.getAddressesWithdrawnOnBlock(
          ...
          for address in addrList:
            ...
            let expectedAccountBalance = ws.wdHistory.getExpectedAccountBalance(
              address, env.clMock.latestExecutedPayload.blockNumber.uint64 - 1
            )
      ...
      onForkchoiceBroadcast: proc(): bool =
        ...
        if not ws.skipBaseVerifications:
          ...
          for address in addrList:
            ...
            #)
            let expectedAccountBalance = ws.wdHistory.getExpectedAccountBalance(
              address, env.clMock.latestExecutedPayload.blockNumber.uint64
            )

            r.expectBalanceEqual(expectedAccountBalance)

          let wds = ws.wdHistory.getWithdrawals(
            env.clMock.latestExecutedPayload.blockNumber.uint64
          )
