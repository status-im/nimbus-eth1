...
type Vault* = ref object # This tracks the account nonce of the vault account.
  nonce: AccountNonce
...
proc newVault*(chainID: ChainID, gasPrice: GasInt, client: RpcClient): Vault =
  ...
  result.vaultKey = PrivateKey
    .fromHex("63b508a03c3b5937ceb903af8b1b0c191012ef6eb7e9c3fb7afa94e5d214d376")
    .get()
...
proc makeFundingTx*(
    v: Vault, recipient: EthAddress, amount: UInt256
): PooledTransaction =
...
proc signTx*(
  ...
  PooledTransaction(tx: signTransaction(unsignedTx, key, v.chainId, eip155 = true))
...
proc createAccount*(v: Vault, amount: UInt256): Future[EthAddress] {.async.} =
  ...
  raise newException(
    ValueError, "could not fund account $2 in transaction $2" % [address.toHex, txHash]
  )
