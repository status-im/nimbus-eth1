...
when CoreDbEnableProfiling or LedgerEnableApiProfiling:
  import std/sequtils

when CoreDbEnableProfiling:
  import
    ../../nimbus/db/aristo/[aristo_api, aristo_profile], ../../nimbus/db/kvt/kvt_api
  var
...
when LedgerEnableApiProfiling:
  import ../../nimbus/db/ledger/base/base_helpers
  var ldgProfData: LedgerProfListRef

const EnableExtraLoggingControl = true
var
...
proc test_chainSyncProfilingPrint*(noisy = false, nBlocks: int, indent = 2) =
  if noisy:
    let info =
      if 0 < nBlocks and nBlocks < high(int):
        " (" & $nBlocks & " blocks)"
      else:
        ""
    discard info
...
proc test_chainSync*(
  ...
  template sayPerf() =
    if blocks > 0:
      ...
      noisy.say "",
        &"{blocks:3} blocks, {(done-sample):2.3}s,",
  ...
  for w in files.undumpBlocks(least = start):
    ...
    if lastOneExtra:
      let blocks0 = blocks9[0 .. 0]
      if oldLogAlign:
        noisy.whisper "***", &"processing {dotsOrSpace}[#{fromBlock},#{lastBlock-1}]\n"
      else:
        sayPerf
        noisy.whisper "***",
      ...
      if oldLogAlign:
        noisy.whisper "***", &"processing {dotsOrSpace}[#{lastBlock},#{toBlock}]\n"
      else:
        sayPerf
