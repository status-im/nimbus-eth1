...
const
  MaxFilterBulk = 150_000 ## Policy settig for `pack()`
...
proc randomisedLeafs(
    db: AristoDbRef, ltys: HashSet[LeafTie], td: var PrngDesc
  ...
  var lvp2 = lvp.sorted(
    cmp = proc(a, b: (LeafTie, RootedVertexID)): int =
      cmp(a[0], b[0])
  )
  if 2 < lvp2.len:
  ...
  let
    ...
    filterMeter =
      if db.balancer.isNil:
        ...
        db.balancer.sTab.len + db.balancer.kMap.len
...
proc saveToBackend(tx: var AristoTxRef, relax: bool, noisy: bool, debugID: int): bool =
...
proc saveToBackendWithOops(
    tx: var AristoTxRef, noisy: bool, debugID: int, oops: (int, AristoError)
): bool =
  var db = tx.to(AristoDbRef)
...
proc testTxMergeAndDeleteOneByOne*(
  ...
  for n, w in list:
    ...
    let (saveMod, saveRest, relax) = block:
      if leafVidPairs.len < 17:
        (7, 3, false)
      elif leafVidPairs.len < 31:
        (11, 7, false)
      else:
        (leafVidPairs.len div 5, 11, true)
    ...
    when true and false:
      noisy.say "***", "del1by1(9)", " n=", n, "/", list.len, " nLeafs=", kvpLeafs.len
