...
suite "Block access list retention period":
  ...
  # BAL_RETENTION_EPOCHS (33024) epochs of the head slot. An epoch is
  # SLOTS_PER_EPOCH (32) slots, so the retention window is 33024 * 32 = 1056768
  ...
  const retentionSlots = BAL_RETENTION_EPOCHS * SLOTS_PER_EPOCH # 1056768
  ...
  test "current and future blocks are within the retention period":
    const headSlot = retentionSlots + 1_000_000'u64
    check:
  ...
  test "the oldest in-window epoch is retained, the next one out is pruned":
    # Epoch-aligned head so the boundary is exact: the oldest retained epoch is
    # headEpoch - (BAL_RETENTION_EPOCHS - 1), and the epoch before it is pruned.
    const
      headEpoch = BAL_RETENTION_EPOCHS + 4000
      headSlot = headEpoch * SLOTS_PER_EPOCH
      oldestRetainedEpoch = headEpoch - (BAL_RETENTION_EPOCHS - 1) # 4001
      firstPrunedEpoch = oldestRetainedEpoch - 1                   # 4000
    check:
      # First and last slot of the oldest retained epoch.
      headerWithSlot(oldestRetainedEpoch * SLOTS_PER_EPOCH)
        .isWithinBalRetentionPeriod(headSlot)
      headerWithSlot(oldestRetainedEpoch * SLOTS_PER_EPOCH + (SLOTS_PER_EPOCH - 1))
        .isWithinBalRetentionPeriod(headSlot)
      # First and last slot of the first pruned epoch, and the genesis slot.
      not headerWithSlot(firstPrunedEpoch * SLOTS_PER_EPOCH)
        .isWithinBalRetentionPeriod(headSlot)
      not headerWithSlot(firstPrunedEpoch * SLOTS_PER_EPOCH + (SLOTS_PER_EPOCH - 1))
        .isWithinBalRetentionPeriod(headSlot)
      not headerWithSlot(0).isWithinBalRetentionPeriod(headSlot)

  test "a block exactly the retention window back is pruned":
    const headSlot = retentionSlots + 1_000_000'u64 # head epoch 64274
    let blockSlot = headSlot - retentionSlots # 1000000, epoch 31250
    check:
  ...
  test "the boundary is by epoch, not raw slot distance":
    # Mid-epoch head. A block in epoch headEpoch - BAL_RETENTION_EPOCHS is exactly
    # BAL_RETENTION_EPOCHS epochs back and pruned even though it is fewer than
    # `retentionSlots` slots behind.
    const
      headEpoch = BAL_RETENTION_EPOCHS + 5000
      headSlot = headEpoch * SLOTS_PER_EPOCH + 5
    let
      prunedSlot =
        (headEpoch - BAL_RETENTION_EPOCHS) * SLOTS_PER_EPOCH + (SLOTS_PER_EPOCH - 1)
      retainedSlot = (headEpoch - BAL_RETENTION_EPOCHS + 1) * SLOTS_PER_EPOCH
    check:
