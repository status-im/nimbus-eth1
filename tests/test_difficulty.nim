...
const inputPath = "tests" / "fixtures" / "eth_tests" / "DifficultyTests"
...
proc parseHash(data: string): Hash256 =
  case data
  of "0x00":
    result = EMPTY_UNCLE_HASH
  of "0x01":
    result.data[0] = 1.byte
  else:
...
proc calculator(
    revision: string, timestamp: EthTime, header: BlockHeader
): DifficultyInt =
  case revision
  of "Homestead":
    result = calcDifficultyHomestead(timestamp, header)
  of "GrayGlacier":
    result = calcDifficultyGrayGlacier(timestamp, header)
  of "Frontier":
    result = calcDifficultyFrontier(timestamp, header)
  of "Berlin":
    result = calcDifficultyMuirGlacier(timestamp, header)
  of "Constantinople":
    result = calcDifficultyConstantinople(timestamp, header)
  of "Byzantium":
    result = calcDifficultyByzantium(timestamp, header)
  of "ArrowGlacier":
    result = calcDifficultyArrowGlacier(timestamp, header)
  else:
