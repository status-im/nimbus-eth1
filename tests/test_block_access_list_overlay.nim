...
suite "Block access list overlay":
  ...
  test "Binary search helpers":
    ...
    check:
      ...
      bal[][accPos].storageReads.findStorageRead(slot2) == 0
      bal[][accPos].storageReads.findStorageRead(slot3) == -1
    ...
    check:
      ...
      balanceChanges.findWriteAt(0) == -1
      balanceChanges.findWriteAt(1) == 0
      balanceChanges.findWriteAt(2) == -1
      balanceChanges.findWriteAt(3) == 1
