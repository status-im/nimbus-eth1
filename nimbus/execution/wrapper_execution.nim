...
proc setupP2P(
    nimbus: NimbusNode, conf: NimbusConf, com: CommonRef
) {.raises: [OSError].} =
  ## Creating P2P Server
  ...
  var address =
    enode.Address(ip: conf.listenAddress, tcpPort: conf.tcpPort, udpPort: conf.udpPort)
  ...
  nimbus.beaconSyncRef = BeaconSyncRef.init(nimbus.ethNode, nimbus.fc, conf.maxPeers)
  ...
  if conf.maxPeers > 0:
    nimbus.networkLoop = nimbus.ethNode.connectToNetwork(
      enableDiscovery = conf.discovery != DiscoveryType.None, waitForPeers = true
    )
...
proc run*(nimbus: NimbusNode, conf: NimbusConf) =
  ...
  let coreDB =
    ...
    AristoDbRocks.newCoreDbRef(
      string conf.dataDir, conf.dbOptions(noKeyCache = conf.cmd == NimbusCmd.`import`)
    )
  ...
  if conf.extraData.len > 32:
    warn "ExtraData exceeds 32 bytes limit, truncate",
      extraData = conf.extraData, len = conf.extraData.len

  if conf.gasLimit > GAS_LIMIT_MAXIMUM or conf.gasLimit < GAS_LIMIT_MINIMUM:
    warn "GasLimit not in expected range, truncate",
      min = GAS_LIMIT_MINIMUM, max = GAS_LIMIT_MAXIMUM, get = conf.gasLimit
