...
import eth/common, results, ./desc_error

type
  GetKvpFn* = proc(key: openArray[byte]): Result[Blob, KvtError] {.gcsafe, raises: [].}
  ...
  PutKvpFn* = proc(hdl: PutHdlRef, kvps: openArray[(Blob, Blob)]) {.gcsafe, raises: [].}
  ...
  CanModFn* = proc(): Result[void, KvtError] {.gcsafe, raises: [].}
  ...
  SetWrReqFn* = proc(db: RootRef): Result[void, KvtError] {.gcsafe, raises: [].}
