...
const extraTraceMessages = false or true ## Enabled additional logging noise
...
proc putEndTriggeredFn(db: RdbBackendRef): PutEndFn =
  ...
  result = proc(hdl: PutHdlRef): Result[void, KvtError] =
    ...
    if hdl.error != KvtError(0):
      when extraTraceMessages:
        debug logTxt "putEndTriggeredFn: failed", error = hdl.error, info = hdl.info
...
proc writeEvCb(db: RdbBackendRef): RdbWriteEventCb =
  ...
  result = proc(ws: WriteBatchRef): bool =
...
proc rocksDbKvtBackend*(
    path: string, dbOpts: DbOptionsRef, cfOpts: ColFamilyOptionsRef
...
proc rocksDbKvtTriggeredBackend*(
  ...
  let db = RdbBackendRef(beKind: BackendRdbTriggered)
...
iterator walk*(be: RdbBackendRef): tuple[key: Blob, data: Blob] =
