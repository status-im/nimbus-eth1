...
proc nearbyNext(
  ...
  # Some easy cases
  let hike = block:
    var rc = hike.zeroAdjust(db, noisy, doLeast=moveRight)
    if rc.isErr:
  ...
  if hike.legs[^1].wp.vtx.vType == Extension:
    ...
    if noisy: echo ">>> nearbyNext (4)",
      " vid=", vid.pp
    return hike.complete(vid, db, noisy, hikeLenMax, doLeast=moveRight)
  ...
  while 0 < uHike.legs.len:
    ...
    if start:
      ...
      of Leaf:
        ...
        if uHike.accept vtx.lPfx:
          return uHike.complete(vid, db, noisy, hikeLenMax, doLeast=moveRight)
      of Extension:
    ...
    if 0 <= n:
      ...
      return uHike.complete(
        step.wp.vtx.bVid[n], db, noisy, hikeLenMax, doLeast=moveRight)
  ...
  result = hike.finalise(db, noisy, moveRight)
...
proc nearbyNextLeafTie(
  ...
  let hike = block:
    let rc = lty.hikeUp(db).to(Hike).nearbyNext(db,noisy, hikeLenMax, moveRight)
    if rc.isErr:
...
proc right*(
  ...
  result = hike.nearbyNext(db, noisy, 64, moveRight=true)
...
proc right*(
  ...
  let rc = lty.nearbyNextLeafTie(db, noisy, 64, moveRight=true)
  if rc.isErr:
...
proc left*(
  ...
  hike.nearbyNext(db, noisy, 64, moveRight=false)
...
proc left*(
  ...
  ok LeafTie(
    root: lty.root,
    path: ? lty.nearbyNextLeafTie(db, noisy, 64, moveRight=false))
