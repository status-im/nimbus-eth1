...
proc vidFetch*(db: AristoDbRef): VertexID =
  ...
  case beDb.vidGen.len:
  of 0:
    beDb.vidGen = @[2.VertexID]
    result = 1.VertexID
  of 1:
    result = beDb.vidGen[^1]
    beDb.vidGen = @[(result.uint64 + 1).VertexID]
  else:
    result = beDb.vidGen[^2]
    beDb.vidGen[^2] = beDb.vidGen[^1]
    beDb.vidGen.setLen(beDb.vidGen.len-1)
...
proc vidDispose*(db: AristoDbRef; vtxID: VertexID) =
  ...
  if beDb.vidGen.len == 0:
    beDb.vidGen = @[vtxID]
  else:
    let topID = beDb.vidGen[^1]
    # No need to store smaller numbers: all numberts larger than `topID`
    # are free numbers
    if vtxID < topID:
      beDb.vidGen[^1] = vtxID
      beDb.vidGen.add topID
