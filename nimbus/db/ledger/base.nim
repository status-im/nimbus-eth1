...
type ReadOnlyStateDB* = distinct LedgerRef

export code_bytes, LedgerRef, LedgerSpRef
...
when LedgerEnableApiProfiling:
  ...
  export LedgerFnInx, LedgerProfListRef
...
proc accessList*(ldg: LedgerRef, eAddr: EthAddress, slot: UInt256) =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), slot
...
proc accountExists*(ldg: LedgerRef, eAddr: EthAddress): bool =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), result
...
proc addBalance*(ldg: LedgerRef, eAddr: EthAddress, delta: UInt256) =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), delta
...
proc getCodeHash*(ldg: LedgerRef, eAddr: EthAddress): Hash256 =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), result = ($$result)
...
proc getCommittedStorage*(ldg: LedgerRef, eAddr: EthAddress, slot: UInt256): UInt256 =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), slot, result
...
proc isTopLevelClean*(ldg: LedgerRef): bool =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, result
...
proc persist*(ldg: LedgerRef, clearEmptyAccount = false, clearCache = false) =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, clearEmptyAccount, clearCache
...
proc setNonce*(ldg: LedgerRef, eAddr: EthAddress, nonce: AccountNonce) =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), nonce
...
proc setStorage*(ldg: LedgerRef, eAddr: EthAddress, slot, val: UInt256) =
  ...
  ldg.ifTrackApi:
    debug apiTxt, api, elapsed, eAddr = ($$eAddr), slot, val

proc setTransientStorage*(
    ldg: LedgerRef, eAddr: EthAddress, slot: UInt256, val: UInt256
) =
...
proc rootHash*(db: ReadOnlyStateDB): KeccakHash =
  db.LedgerRef.state()
...
proc getStorage*(
  db: ReadOnlyStateDB, eAddr: EthAddress, slot: UInt256
): UInt256 {.borrow.}
