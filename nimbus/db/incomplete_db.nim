...
proc populateDbWithBranch*(db: TrieDatabaseRef, branch: seq[seq[byte]]) =
  for nodeBytes in branch:
    ...
    # FIXME-Adam - feel free to delete this, I was just trying to understand whether we're receiving extension nodes
    # let nodeRlp = rlpFromBytes(nodeBytes)
    # if nodeRlp.listLen == 2:
    #   let (isLeaf, k) = nodeRlp.extensionNodeKey
    #   warn("populateDbWithBranch, found a length 2 list", nodeHash=nodeHash, isLeaf=isLeaf, k=k)
