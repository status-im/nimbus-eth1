...
type TransactionKey = tuple[blockNumber: BlockNumber, index: uint]
...
proc getBlockHeader*(
  db: CoreDbRef, n: BlockNumber, output: var BlockHeader
): bool {.gcsafe.}

proc getBlockHeader*(
  db: CoreDbRef, blockHash: Hash256
): BlockHeader {.gcsafe, raises: [BlockNotFound].}

proc getBlockHash*(db: CoreDbRef, n: BlockNumber, output: var Hash256): bool {.gcsafe.}
...
proc getBlockHeader*(
  db: CoreDbRef, blockHash: Hash256, output: var BlockHeader
): bool {.gcsafe.}
...
iterator findNewAncestors(db: CoreDbRef, header: BlockHeader): BlockHeader =
...
iterator getBlockTransactionData*(db: CoreDbRef, txRoot: Hash256): Blob =
  block body:
...
iterator getBlockTransactions*(db: CoreDbRef, header: BlockHeader): Transaction =
  for encodedTx in db.getBlockTransactionData(header.txRoot):
    try:
      yield rlp.decode(encodedTx, Transaction)
    except RlpError as exc:
      warn logTxt "Cannot decode database transaction",
        data = toHex(encodedTx), error = exc.msg

iterator getBlockTransactionHashes*(db: CoreDbRef, blockHeader: BlockHeader): Hash256 =
...
iterator getWithdrawals*(
    db: CoreDbRef, withdrawalsRoot: Hash256
...
iterator getReceipts*(
    db: CoreDbRef, receiptsRoot: Hash256
): Receipt {.gcsafe, raises: [RlpError].} =
  block body:
...
proc removeTransactionFromCanonicalChain(db: CoreDbRef, transactionHash: Hash256) =
...
proc setAsCanonicalChainHead(db: CoreDbRef, headerHash: Hash256, header: BlockHeader) =
...
proc markCanonicalChain(
    db: CoreDbRef, header: BlockHeader, headerHash: Hash256
): bool {.gcsafe, raises: [RlpError].} =
...
proc getSavedStateBlockNumber*(db: CoreDbRef): BlockNumber =
  ## Returns the block number registered when the database was last time
  ## updated, or `BlockNumber(0)` if there was no updata found.
  ##
  db.stateBlockNumber()

proc getBlockHeader*(db: CoreDbRef, blockHash: Hash256, output: var BlockHeader): bool =
...
proc getBlockHeader*(db: CoreDbRef, blockHash: Hash256): BlockHeader =
...
proc getHash(db: CoreDbRef, key: DbKey): Opt[Hash256] =
...
proc getCanonicalHead*(db: CoreDbRef, output: var BlockHeader): bool =
...
proc getCanonicalHead*(db: CoreDbRef): BlockHeader {.gcsafe, raises: [EVMError].} =
  if not db.getCanonicalHead result:
    raise newException(CanonicalHeadNotFound, "No canonical head set for this chain")

proc getBlockHash*(db: CoreDbRef, n: BlockNumber, output: var Hash256): bool =
...
proc getBlockHash*(
    db: CoreDbRef, n: BlockNumber
): Hash256 {.gcsafe, raises: [BlockNotFound].} =
  ## Return the block hash for the given block number.
...
proc getBlockHeader*(db: CoreDbRef, n: BlockNumber, output: var BlockHeader): bool =
...
proc getBlockHeaderWithHash*(
  ...
  if db.getBlockHash(n, hash):
    ...
    if db.getBlockHeader(hash, header):
      ...
      raiseAssert(
        "Corrupted database. Mapping number->hash present, without header in database"
      )
  else:
...
proc getBlockHeader*(
    db: CoreDbRef, n: BlockNumber
): BlockHeader {.raises: [BlockNotFound].} =
  ## Returns the block header with the given number in the canonical chain.
  ## Raises BlockNotFound error if the block is not in the DB.
  db.getBlockHeader(db.getBlockHash(n))

proc getScore*(db: CoreDbRef, blockHash: Hash256): Opt[UInt256] =
  let data = db.ctx.getKvt().get(blockHashToScoreKey(blockHash).toOpenArray).valueOr:
      if error.error != KvtNotFound:
...
proc headTotalDifficulty*(db: CoreDbRef): UInt256 =
...
proc getAncestorsHashes*(
    db: CoreDbRef, limit: BlockNumber, header: BlockHeader
): seq[Hash256] {.gcsafe, raises: [BlockNotFound].} =
...
proc forgetHistory*(db: CoreDbRef, blockNum: BlockNumber): bool =
...
proc getTransactionByIndex*(
    db: CoreDbRef, txRoot: Hash256, txIndex: uint16, res: var Transaction
): bool =
  const info = "getTransaction()"
  ...
  try:
    ...
    warn logTxt info, txRoot, action = "rlp.decode()", error = exc.msg
    return false
  true

proc getTransactionCount*(db: CoreDbRef, txRoot: Hash256): int =
  const info = "getTransactionCount()"
  ...
  while true:
    ...
    let yes = kvt.hasKey(key).valueOr:
      warn logTxt info, txRoot, key, action = "hasKey()", error = ($$error)
      return 0
    if yes:
      inc txCount
    else:
...
proc getUnclesCount*(
    db: CoreDbRef, ommersHash: Hash256
): int {.gcsafe, raises: [RlpError].} =
...
proc getUncles*(
    db: CoreDbRef, ommersHash: Hash256
): seq[BlockHeader] {.gcsafe, raises: [RlpError].} =
...
proc persistWithdrawals*(
    db: CoreDbRef, withdrawalsRoot: Hash256, withdrawals: openArray[Withdrawal]
) =
...
proc getWithdrawals*(
    db: CoreDbRef, withdrawalsRoot: Hash256
): seq[Withdrawal] {.gcsafe, raises: [RlpError].} =
  for wd in db.getWithdrawals(withdrawalsRoot):
    result.add(wd)

proc getTransactions*(
    db: CoreDbRef, txRoot: Hash256, output: var seq[Transaction]
) {.gcsafe, raises: [RlpError].} =
  for encodedTx in db.getBlockTransactionData(txRoot):
    output.add(rlp.decode(encodedTx, Transaction))

proc getTransactions*(
    db: CoreDbRef, txRoot: Hash256
): seq[Transaction] {.gcsafe, raises: [RlpError].} =
  db.getTransactions(txRoot, result)

proc getBlockBody*(
    db: CoreDbRef, header: BlockHeader, output: var BlockBody
): bool {.gcsafe, raises: [RlpError].} =
...
proc getBlockBody*(
    db: CoreDbRef, blockHash: Hash256, output: var BlockBody
): bool {.gcsafe, raises: [RlpError].} =
  var header: BlockHeader
  if db.getBlockHeader(blockHash, header):
...
proc getBlockBody*(
    db: CoreDbRef, hash: Hash256
): BlockBody {.gcsafe, raises: [RlpError, ValueError].} =
  if not db.getBlockBody(hash, result):
    raise newException(ValueError, "Error when retrieving block body")

proc getEthBlock*(
    db: CoreDbRef, hash: Hash256
): EthBlock {.gcsafe, raises: [BlockNotFound, RlpError, ValueError].} =
  var
...
proc getUncleHashes*(
    db: CoreDbRef, blockHashes: openArray[Hash256]
): seq[Hash256] {.gcsafe, raises: [RlpError, ValueError].} =
  for blockHash in blockHashes:
    result &= db.getBlockBody(blockHash).uncles.mapIt(it.rlpHash)

proc getUncleHashes*(
    db: CoreDbRef, header: BlockHeader
): seq[Hash256] {.gcsafe, raises: [RlpError].} =
...
proc getTransactionKey*(
    db: CoreDbRef, transactionHash: Hash256
): tuple[blockNumber: BlockNumber, index: uint64] {.gcsafe, raises: [RlpError].} =
  let
...
proc setHead*(db: CoreDbRef, blockHash: Hash256): bool {.gcsafe, raises: [RlpError].} =
  var header: BlockHeader
...
proc setHead*(
    db: CoreDbRef, header: BlockHeader, writeHeader = false
): bool {.gcsafe, raises: [RlpError].} =
...
proc persistReceipts*(
    db: CoreDbRef, receiptsRoot: Hash256, receipts: openArray[Receipt]
) =
...
proc getReceipts*(
    db: CoreDbRef, receiptsRoot: Hash256
): seq[Receipt] {.gcsafe, raises: [RlpError].} =
  var receipts = newSeq[Receipt]()
  for r in db.getReceipts(receiptsRoot):
    receipts.add(r)
  return receipts

proc persistScore*(db: CoreDbRef, blockHash: Hash256, score: UInt256): bool =
  let
    kvt = db.ctx.getKvt()
    scoreKey = blockHashToScoreKey(blockHash)
  kvt.put(scoreKey.toOpenArray, rlp.encode(score)).isOkOr:
    warn logTxt "persistHeader()", scoreKey, action = "put()", `error` = ($$error)
    return
  true
...
proc safeHeaderHash*(db: CoreDbRef, headerHash: Hash256) =
  ...
  db.ctx.getKvt.put(safeHashKey.toOpenArray, rlp.encode(headerHash)).isOkOr:
    warn logTxt "safeHeaderHash()", safeHashKey, action = "put()", `error` = ($$error)
    return

proc finalizedHeaderHash*(db: CoreDbRef): Hash256 =
...
proc safeHeader*(db: CoreDbRef): BlockHeader {.gcsafe, raises: [BlockNotFound].} =
  db.getBlockHeader(db.safeHeaderHash)

proc finalizedHeader*(db: CoreDbRef): BlockHeader {.gcsafe, raises: [BlockNotFound].} =
