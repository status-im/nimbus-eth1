...
proc sumHash*(header: BlockHeader): Hash256 =
  let wdRoot =
    if header.withdrawalsRoot.isSome: header.withdrawalsRoot.get else: EMPTY_ROOT_HASH
  sumHash(header.txRoot, header.ommersHash, wdRoot)

func hasBody*(h: BlockHeader): bool =
  h.txRoot != EMPTY_ROOT_HASH or h.ommersHash != EMPTY_UNCLE_HASH or
...
func generateSafeAddress*(
    address: EthAddress, salt: ContractSalt, data: openArray[byte]
): EthAddress =
...
proc crc32*(crc: uint32, buf: openArray[byte]): uint32 =
  const kcrc32 = [
    0'u32, 0x1db71064, 0x3b6e20c8, 0x26d930ac, 0x76dc4190, 0x6b6b51f4, 0x4db26158,
    0x5005713c, 0xedb88320'u32, 0xf00f9344'u32, 0xd6d6a3e8'u32, 0xcb61b38c'u32,
  ...
  for b in buf:
    crcu32 =
      (crcu32 shr 4) xor kcrc32[int((crcu32 and 0xF) xor (uint32(b) and 0xF'u32))]
...
proc decompose*(
    rlp: var Rlp, header: var BlockHeader, body: var BlockBody
...
proc decompose*(
    rlpBytes: openArray[byte], header: var BlockHeader, body: var BlockBody
) {.gcsafe, raises: [RlpError].} =
...
func isGenesis*(header: BlockHeader): bool =
  header.number == 0'u64 and header.parentHash == GENESIS_PARENT_HASH
