...
type Op* = enum
  ...
  IsZero = 0x15
    ## Simple not operator. (Note: real Yellow Paper
    ## description)
  ...
  CallDataCopy = 0x37
    ## Copy input data in current environment to
    ## memory.
  CodeSize = 0x38 ## Get size of code running in current environment.
  CodeCopy = 0x39
    ## Copy code running in current environment to
    ## memory.
  ...
  ReturnDataCopy = 0x3e
    ## Copy output data from the previous call to
    ## memory.
  ...
  Gas = 0x5a
    ...
    ## instruction.
  JumpDest = 0x5b
    ## Mark a valid destination for jumps. This
  ...
  Nop0xFC ## ..
  Revert = 0xfd
    ## Halt execution reverting state changes but
    ## returning data and remaining gas.
  Invalid = 0xfe ## Designated invalid instruction.
  SelfDestruct = 0xff
    ## Halt execution and register account for later
    ## deletion.
...
static:
  ...
  proc badSymNames(a: openArray[string]): seq[string] =
    toSeq(a).filterIt(
      it.len < 2 or not it[0].isUpperAscii or it.count(allowedChars) < it.len
    )
  ...
  when vfySymNames.len != 254:
    static:
      echo "*** Got only ", vfySymNames.len, " rejected Vfy enum symbols (expected 254)"
