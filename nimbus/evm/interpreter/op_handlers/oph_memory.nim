...
when evmc_enabled:
  proc sstoreEvmc(
      c: Computation, slot, newValue: UInt256, coldAccess = 0.GasInt
  ): EvmResultVoid =
    let
  ...
  proc sstoreImpl(c: Computation, slot, newValue: UInt256): EvmResultVoid =
    let
      ...
      gasParam = GasParamsSs(currentValue: currentValue)
...
template sstoreEvmcOrNetGasMetering(
    cpt, slot, newValue: untyped, coldAccess = 0.GasInt
): auto =
  when evmc_enabled:
...
const VmOpExecMemory*: seq[VmOpExec] =
  @[
    (
