...
type
  ...
  GasCost = object
    ...
    of GckFixed, GckLater:
      cost*: GasInt
    of GckDynamic:
      d_handler*: proc(value: UInt256): GasInt {.nimcall, gcsafe, raises: [].}
    of GckMemExpansion:
    ...
    of GckSuicide:
      sc_handler*: proc(condition: bool): GasInt {.nimcall, gcsafe, raises: [].}
    of GckSstore:
...
template gasCosts(fork: EVMFork, prefix, ResultGasCostsName: untyped) =
  ...
  func `prefix gasMemoryExpansion`(
      currentMemSize, memOffset, memLength: GasNatural
  ): GasInt {.inline.} =
    # Input: size (in bytes)
    ...
    let
      prevCost = prevWords * static(FeeSchedule[GasMemory]) + (prevWords ^ 2) shr 9
        # div 512
      newCost = newWords * static(FeeSchedule[GasMemory]) + (newWords ^ 2) shr 9
        # div 512
    ...
    if value.isZero:
      ...
      result =
        ...
        `prefix gasMemoryExpansion`(
          params.currentMemSize, params.memOffset, params.memLength
        )
  ...
  func `prefix gasSstore`(
      value: UInt256, params: GasParamsSs
  ): SStoreGasResult {.nimcall.} =
    ...
    const
      ...
      InitRefund {.used.} = FeeSchedule[GasSset] - SLOAD_GAS
        # resetting to the original zero value
      CleanGas {.used.} =
        GasInt SSTORE_RESET_GAS # from clean non-zero to something else
      CleanRefund {.used.} = SSTORE_RESET_GAS - SLOAD_GAS
        # resetting to the original non-zero value
      ClearRefund {.used.} = FeeSchedule[RefundsClear]
        # clearing an originally existing storage slot
    ...
    when fork < FkConstantinople or fork == FkPetersburg:
      ...
      res.gasCost = if value.isZero.not and isStorageEmpty: InitGas else: CleanGas
  ...
  func `prefix gasLog0`(
    ...
    result +=
      GasInt(static(FeeSchedule[GasLog]) + static(FeeSchedule[GasLogData]) * memLength)
  ...
  func `prefix gasLog1`(
    ...
    result +=
      GasInt(
        static(FeeSchedule[GasLog]) + static(FeeSchedule[GasLogData]) * memLength +
  ...
  func `prefix gasCall`(
      value: UInt256, params: GasParams
  ): EvmResult[CallGasResult] {.nimcall.} =
  ...
  let `ResultGasCostsName`* {.inject, compileTime.}: GasCosts = block:
    ...
    func dynamic(
        handler: proc(value: UInt256): GasInt {.nimcall, gcsafe, raises: [].}
    ): GasCost =
      GasCost(kind: GckDynamic, d_handler: handler)

    func memExpansion(
        handler: proc(currentMemSize, memOffset, memLength: GasNatural): GasInt {.
          nimcall, gcsafe, raises: []
        .}
    ): GasCost =
    ...
    func handleCall(
        handler: proc(value: UInt256, gasParams: GasParams): EvmResult[CallGasResult] {.
          nimcall, gcsafe, raises: []
        .}
    ): GasCost =
    ...
    func handleCreate(
        handler: proc(value: UInt256, gasParams: GasParamsCr): GasInt {.
          nimcall, gcsafe, raises: []
        .}
    ): GasCost =
    ...
    func handleSuicide(
        handler: proc(condition: bool): GasInt {.nimcall, gcsafe, raises: [].}
    ): GasCost =
      GasCost(kind: GckSuicide, sc_handler: handler)

    func handleSstore(
        handler: proc(value: UInt256, gasParams: GasParamsSs): SStoreGasResult {.
          nimcall, gcsafe, raises: []
        .}
    ): GasCost =
...
const BaseGasFees: GasFeeSchedule = [
...
func londonGasFees(previousFees: GasFeeSchedule): GasFeeSchedule =
  ...
  result[RefundsClear] = 5000 - ColdSloadCost + ACCESS_LIST_STORAGE_KEY_COST
