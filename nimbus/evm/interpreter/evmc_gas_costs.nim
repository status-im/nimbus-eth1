...
import ../../common/evmforks, evmc/evmc
...
func storageCostSpec(): array[EVMFork, StorageCostSpec] {.compileTime.} =
  ...
  const revs =
    [FkFrontier, FkHomestead, FkTangerine, FkSpurious, FkByzantium, FkPetersburg]
  ...
  result[FkBerlin] = StorageCostSpec(
    netCost: true,
    warmAccess: WarmStorageReadCost,
    sset: 20000,
    reset: 5000 - ColdSloadCost,
    clear: 15000,
  )
...
proc legacySStoreCost(e: var SstoreCosts, c: StorageCostSpec) {.compileTime.} =
...
proc netSStoreCost(e: var SstoreCosts, c: StorageCostSpec) {.compileTime.} =
  e[EVMC_STORAGE_ASSIGNED] = StorageStoreCost(gasCost: c.warmAccess, gasRefund: 0)
  e[EVMC_STORAGE_ADDED] = StorageStoreCost(gasCost: c.sset, gasRefund: 0)
  e[EVMC_STORAGE_DELETED] = StorageStoreCost(gasCost: c.reset, gasRefund: c.clear)
  e[EVMC_STORAGE_MODIFIED] = StorageStoreCost(gasCost: c.reset, gasRefund: 0)
  e[EVMC_STORAGE_DELETED_ADDED] =
    StorageStoreCost(gasCost: c.warmAccess, gasRefund: -c.clear)
  e[EVMC_STORAGE_MODIFIED_DELETED] =
    StorageStoreCost(gasCost: c.warmAccess, gasRefund: c.clear)
  e[EVMC_STORAGE_DELETED_RESTORED] =
    StorageStoreCost(gasCost: c.warmAccess, gasRefund: c.reset - c.warmAccess - c.clear)
  e[EVMC_STORAGE_ADDED_DELETED] =
    StorageStoreCost(gasCost: c.warmAccess, gasRefund: c.sset - c.warmAccess)
  e[EVMC_STORAGE_MODIFIED_RESTORED] =
    StorageStoreCost(gasCost: c.warmAccess, gasRefund: c.reset - c.warmAccess)
...
const ForkToSstoreCost* = storageStoreCost()
