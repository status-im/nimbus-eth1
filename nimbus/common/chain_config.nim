...
proc writeValue(
    writer: var JsonWriter, value: Opt[EthTime]
...
proc append*(w: var RlpWriter, ab: AddressBalance) =
  var listLen = 2
  if ab.account.storage.len > 0 or ab.account.nonce != 0.AccountNonce or
...
proc append*(w: var RlpWriter, ga: GenesisAlloc) =
  ...
  for k, v in ga:
    list.add AddressBalance(address: k, account: v)
  w.append(list)

func decodePrealloc*(data: seq[byte]): GenesisAlloc {.gcsafe, raises: [RlpError].} =
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var UInt256
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var ChainId
) {.gcsafe, raises: [SerializationError, IOError].} =
  value = reader.readValue(int).ChainId

proc readValue(
    reader: var JsonReader[JGenesis], value: var Hash256
) {.gcsafe, raises: [SerializationError, IOError].} =
  value = Hash256.fromHex(reader.readValue(string))

proc readValue(
    reader: var JsonReader[JGenesis], value: var BlockNonce
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var EthTime
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var Opt[EthTime]
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var seq[byte]
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
    value = hexToSeqByte(reader.readValue(string))

proc readValue(
    reader: var JsonReader[JGenesis], value: var EthAddress
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
    value = parseAddress(reader.readValue(string))

proc readValue(
    reader: var JsonReader[JGenesis], value: var uint64
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var GenesisStorage
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
...
proc readValue(
    reader: var JsonReader[JGenesis], value: var GenesisAlloc
) {.gcsafe, raises: [SerializationError, IOError].} =
  wrapError:
    for key in reader.readObjectFields:
...
macro fillArrayOfBlockNumberBasedForkOptionals(conf, tmp: typed): untyped =
  ...
  for i, x in forkBlockField:
    ...
    result.add quote do:
      `tmp`[`i`] = BlockNumberBasedForkOptional(number: `conf`.`fieldIdent`, name: `x`)
...
macro fillArrayOfTimeBasedForkOptionals(conf, tmp: typed): untyped =
  ...
  for i, x in forkTimeField:
    let fieldIdent = newIdentNode(x)
    result.add quote do:
      `tmp`[`i`] = TimeBasedForkOptional(time: `conf`.`fieldIdent`, name: `x`)
...
func toHardFork*(
    map: ForkTransitionTable, forkDeterminer: ForkDeterminationInfo
): HardFork =
...
proc validateChainConfig*(conf: ChainConfig): bool =
  ...
  var blockNumberBasedForkOptionals:
...
proc parseGenesis*(data: string): Genesis {.gcsafe.} =
  try:
    ...
    error "Error loading genesis data", exception = e.name, msg = e.msg
    return nil

proc parseGenesisFile*(fileName: string): Genesis {.gcsafe.} =
  try:
    ...
    error "Error loading genesis file", fileName, exception = e.name, msg = e.msg
    return nil

proc validateNetworkParams(
    params: var NetworkParams, input: string, inputIsFile: bool
): bool =
  if params.genesis.isNil:
    # lets try with geth's format
    let genesis =
      if inputIsFile:
        parseGenesisFile(input)
      else:
        parseGenesis(input)
    if genesis.isNil:
      return false
...
proc loadNetworkParams*(fileName: string, params: var NetworkParams): bool =
  try:
    ...
    error "Error loading network params file", fileName, exception = e.name, msg = e.msg
    return false
...
proc parseGenesisAlloc*(
    data: string, ga: var GenesisAlloc
  ...
  result =
    ...
    of MainNet:
      ...
      ChainConfig(
        ...
        eip150Hash:
          toDigest("2086799aeebeae135c246c65021c82b4e15a2c451340993aacfd2751886514f0"),
...
func genesisBlockForNetwork*(
    id: NetworkId
): Genesis {.gcsafe, raises: [ValueError, RlpError].} =
  result =
    case id
    of MainNet:
      Genesis(
...
func networkParams*(
    id: NetworkId
): NetworkParams {.gcsafe, raises: [ValueError, RlpError].} =
...
func `==`*(a, b: Genesis): bool =
  if a.isNil and b.isNil:
    return true
  if a.isNil and not b.isNil:
    return false
  if not a.isNil and b.isNil:
    return false
  a[] == b[]
