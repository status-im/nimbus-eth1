...
from stew/byteutils import nil

export results

const daoForkBlockExtraData* = byteutils.hexToByteArray[13](DAOForkBlockExtra).toSeq
...
proc validateHeader(
    com: CommonRef, blk: EthBlock, parentHeader: BlockHeader, checkSealOK: bool
): Result[void, string] =
  ...
  template inDAOExtraRange(blockNumber: BlockNumber): bool =
    ...
    daoForkBlock <= blockNumber and blockNumber < DAOHigh
...
proc validateUncles(
  ...
  for uncle in uncles:
    ...
    ?com.validateHeader(EthBlock.init(uncle, BlockBody()), uncleParent, checkSealOK)
...
func gasCost*(tx: Transaction): UInt256 =
  if tx.txType >= TxEip4844:
    tx.gasLimit.u256 * tx.maxFeePerGas.u256 +
      tx.getTotalBlobGas.u256 * tx.maxFeePerBlobGas
...
proc validateTxBasic*(
  ...
  try:
    ...
    if tx.maxFeePerGas < tx.maxPriorityFeePerGas:
      return err(
        "invalid tx: maxFee is smaller than maPriorityFee. maxFee=$1, maxPriorityFee=$2" %
          [$tx.maxFeePerGas, $tx.maxPriorityFeePerGas]
      )

    if tx.gasLimit < tx.intrinsicGas(fork):
      return err(
        "invalid tx: not enough gas to perform calculation. avail=$1, require=$2" %
    ...
    if fork >= FkCancun:
      if tx.payload.len > MAX_CALLDATA_SIZE:
        return err(
          "invalid tx: payload len exceeds MAX_CALLDATA_SIZE. len=" & $tx.payload.len
        )

      if tx.accessList.len > MAX_ACCESS_LIST_SIZE:
        return err(
          "invalid tx: access list len exceeds MAX_ACCESS_LIST_SIZE. len=" &
            $tx.accessList.len
        )
      ...
      for i, acl in tx.accessList:
        if acl.storageKeys.len > MAX_ACCESS_LIST_STORAGE_KEYS:
          return err(
            "invalid tx: access list storage keys len exceeds MAX_ACCESS_LIST_STORAGE_KEYS. " &
    ...
    if tx.txType >= TxEip4844:
      ...
      if tx.versionedHashes.len > MAX_BLOBS_PER_BLOCK:
        return err(
          "invalid tx: versioned hashes len exceeds MAX_BLOBS_PER_BLOCK=" &
            $MAX_BLOBS_PER_BLOCK & ". get=" & $tx.versionedHashes.len
        )
...
proc validateTransaction*(
  ...
  try:
    if maxLimit < tx.gasLimit:
      return err(
        "invalid tx: block header gasLimit exceeded. maxLimit=$1, gasLimit=$2" %
    ...
    if tx.maxFeePerGas < baseFee.truncate(GasInt):
      return err(
        "invalid tx: maxFee is smaller than baseFee. maxFee=$1, baseFee=$2" %
    ...
    if balance < gasCost:
      return err(
        "invalid tx: not enough cash for gas. avail=$1, require=$2" %
          [$balance, $gasCost]
      )

    if balance - gasCost < tx.value:
      return err(
        "invalid tx: not enough cash to send. avail=$1, availMinusGas=$2, require=$3" %
          [$balance, $(balance - gasCost), $tx.value]
      )

    if tx.nonce != nonce:
      return err(
        "invalid tx: account nonce mismatch. txNonce=$1, accNonce=$2" %
          [$tx.nonce, $nonce]
      )
    ...
    if codeHash != EMPTY_CODE_HASH:
      return err(
        "invalid tx: sender is not an EOA. sender=$1, codeHash=$2" %
    ...
    if tx.txType >= TxEip4844:
      ...
      if tx.maxFeePerBlobGas < blobGasPrice:
        return err(
          "invalid tx: maxFeePerBlobGas smaller than blobGasPrice. " &
...
proc validateHeaderAndKinship*(
    com: CommonRef, blk: EthBlock, parent: BlockHeader, checkSealOK: bool
): Result[void, string] {.gcsafe, raises: [].} =
  template header(): BlockHeader =
    blk.header
