...
proc mkInxImpl(
    sq: var TxStatusTab, item: TxItemRef
): Result[TxStatusInx, void] {.gcsafe, raises: [KeyError].} =
...
proc eq*(
    sq: var TxStatusTab, status: TxItemStatus
...
proc eq*(
    addrData: TxStatusSenderRef, sender: EthAddress
): SortedSetResult[EthAddress, TxStatusNonceRef] {.gcsafe, raises: [KeyError].} =
  if addrData.addrList.hasKey(sender):
    return toSortedSetResult(key = sender, data = addrData.addrList[sender])
  err(rbNotFound)

proc eq*(
    rc: SortedSetResult[TxItemStatus, TxStatusSenderRef], sender: EthAddress
): SortedSetResult[EthAddress, TxStatusNonceRef] {.gcsafe, raises: [KeyError].} =
  if rc.isOk:
...
proc eq*(
    nonceData: TxStatusNonceRef, nonce: AccountNonce
): SortedSetResult[AccountNonce, TxItemRef] =
  nonceData.nonceList.eq(nonce)

proc eq*(
    rc: SortedSetResult[EthAddress, TxStatusNonceRef], nonce: AccountNonce
): SortedSetResult[AccountNonce, TxItemRef] =
  if rc.isOk:
