...
proc resetTxEnv(dh: TxChainRef, parent: BlockHeader) =
  ...
  dh.com.hardForkTransition(parent.blockHash, parent.number + 1, Opt.some(timestamp))
  dh.prepareHeader()
  ...
  dh.vmState = BaseVMState.new(parent = parent, blockCtx = blockCtx, com = dh.com)

  dh.txRoot = EMPTY_ROOT_HASH
  dh.stateRoot = dh.vmState.parent.stateRoot

proc update(dh: TxChainRef, parent: BlockHeader) {.gcsafe, raises: [].} =
  let
    db = dh.com.db
...
proc new*(T: type TxChainRef, com: CommonRef): T {.gcsafe, raises: [EVMError].} =
  ## Constructor
  new result
...
proc getHeader*(dh: TxChainRef): BlockHeader {.gcsafe, raises: [].} =
...
proc clearAccounts*(dh: TxChainRef) {.gcsafe, raises: [].} =
...
proc `head=`*(dh: TxChainRef, val: BlockHeader) {.gcsafe, raises: [].} =
