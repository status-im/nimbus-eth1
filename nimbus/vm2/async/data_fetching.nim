...
proc storeBlockHeader(chainDB: BaseChainDB, header: BlockHeader) =
  chainDB.persistHeaderToDbWithoutSetHeadOrScore(header)

proc assertThatWeHaveStoredBlockHeader(chainDB: BaseChainDB, blockNumber: BlockNumber, header: BlockHeader) =
...
proc ifNecessaryGetBlockHeader*(client: RpcClient, chainDB: BaseChainDB, blockNumber: BlockNumber, justChecking: bool): Future[void] {.async.} =
