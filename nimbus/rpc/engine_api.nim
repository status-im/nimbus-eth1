...
proc txPriorityFee(ttx: TypedTransaction): UInt256 =
  ...
  u256(tx.gasPrice * tx.maxPriorityFee)

proc sumOfBlockPriorityFees(payload: ExecutionPayloadV2): UInt256 =
  payload.transactions.foldl(a + txPriorityFee(b), UInt256.zero)

# FIXME-Adam: How do I do this? IIUC, Shanghai (and later) forks
# happen based on timestamp rather than block number.
#
# The stuff in hardforks.nim and chain_config.nim seems relevant.
const WITHDRAWALS_ACTIVATION_FORK_TIMESTAMP: uint64 = high(uint64)
...
proc handle_newPayload2(sealingEngine: SealingEngineRef, api: EngineApiRef, com: CommonRef, payload: ExecutionPayloadV2): PayloadStatusV1 =
  let areWithdrawalsActivated = uint64(payload.timestamp) >= WITHDRAWALS_ACTIVATION_FORK_TIMESTAMP

  var header = toBlockHeader(areWithdrawalsActivated, payload)
  ...
  if areWithdrawalsActivated:
    if payload.withdrawals.isNone:
      return invalidStatus(latestValidHash(db, parent, ttd), "Null withdrawals")
  else:
    if payload.withdrawals.isSome:
      return invalidStatus(latestValidHash(db, parent, ttd), "Non-null withdrawals even though withdrawal functionality not activated yet")
...
proc handle_getPayloadV2(api: EngineApiRef, payloadId: PayloadID): GetPayloadV2Result =
  var payload: ExecutionPayloadV2
  if not api.get(payloadId, payload):
    raise unknownPayload("Unknown payload")
  let blockValue: Quantity = Quantity(sumOfBlockPriorityFees(payload).truncate(uint64))
  return GetPayloadV2Result(
    executionPayload: payload,
    blockValue: blockValue
  )

# https://github.com/ethereum/execution-apis/blob/main/src/engine/paris.md#engine_exchangetransitionconfigurationv1
proc handle_exchangeTransitionConfigurationV1(sealingEngine: SealingEngineRef, com: CommonRef, conf: TransitionConfigurationV1): TransitionConfigurationV1 =
  let db = sealingEngine.chain.db
  let ttd = com.ttd
...
proc handle_forkchoiceUpdatedV2(
  ...
  if payloadAttributes.isSome:
    ...
    let areWithdrawalsActivated = uint64(payloadAttrs.timestamp) >= WITHDRAWALS_ACTIVATION_FORK_TIMESTAMP
    
    if areWithdrawalsActivated:
      if payloadAttrs.withdrawals.isNone:
        error "Null withdrawals"
        raise invalidAttr("Null withdrawals")
    else:
      if payloadAttrs.withdrawals.isSome:
        error "Non-null withdrawals even though withdrawal functionality not activated yet"
        raise invalidAttr("Non-null withdrawals even though withdrawal functionality not activated yet")
...
proc setupEngineApi*(
  ...
  server.rpc("engine_newPayloadV1") do(payload: ExecutionPayloadV1) -> PayloadStatusV1:
    ...
    return handle_newPayload2(sealingEngine, api, com, payload.toPayloadV2)
  
  server.rpc("engine_newPayloadV2") do(payload: ExecutionPayloadV2) -> PayloadStatusV1:
    trace "Engine API request received",
      meth = "newPayloadV2", number = $(distinctBase payload.blockNumber), hash = payload.blockHash
    return handle_newPayload2(sealingEngine, api, com, payload)

  server.rpc("engine_getPayloadV1") do(payloadId: PayloadID) -> ExecutionPayloadV1:
    trace "Engine API request received",
      meth = "GetPayloadV1", id = payloadId.toHex
    return handle_getPayloadV2(api, payloadId).executionPayload.toPayloadV1

  server.rpc("engine_getPayloadV2") do(payloadId: PayloadID) -> GetPayloadV2Result:
    trace "Engine API request received",
      meth = "GetPayloadV2", id = payloadId.toHex
    return handle_getPayloadV2(api, payloadId)
  ...
  server.rpc("engine_exchangeTransitionConfigurationV1") do(conf: TransitionConfigurationV1) -> TransitionConfigurationV1:
    ...
    return handle_exchangeTransitionConfigurationV1(sealingEngine, com, conf)

  server.rpc("engine_forkchoiceUpdatedV1") do(
      update: ForkchoiceStateV1,
      payloadAttributes: Option[PayloadAttributesV1]) -> ForkchoiceUpdatedResponse:
    return handle_forkchoiceUpdatedV2(sealingEngine, api, com, update, payloadAttributes.map(toPayloadAttributesV2))

  server.rpc("engine_forkchoiceUpdatedV2") do(
      update: ForkchoiceStateV1,
      payloadAttributes: Option[PayloadAttributesV2]) -> ForkchoiceUpdatedResponse:
    return handle_forkchoiceUpdatedV2(sealingEngine, api, com, update, payloadAttributes)
