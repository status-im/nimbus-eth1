...
proc getStorage(
    p: evmc_host_context, address: var evmc_address, key: var evmc_bytes32
): evmc_bytes32 {.cdecl.} =
  toHost(p).getStorage(address.fromEvmc, key.flip256.fromEvmc).toEvmc.flip256

proc setStorage(
    p: evmc_host_context, address: var evmc_address, key, value: var evmc_bytes32
): evmc_storage_status {.cdecl.} =
  toHost(p).setStorage(address.fromEvmc, key.flip256.fromEvmc, value.flip256.fromEvmc)

proc getBalance(
    p: evmc_host_context, address: var evmc_address
): evmc_uint256be {.cdecl.} =
  toHost(p).getBalance(address.fromEvmc).toEvmc.flip256

proc getCodeSize(p: evmc_host_context, address: var evmc_address): csize_t {.cdecl.} =
...
proc selfDestruct(
    p: evmc_host_context, address, beneficiary: var evmc_address
) {.cdecl.} =
...
proc emitLog(
  ...
  toHost(p).emitLog(
    address.fromEvmc, data, data_size, cast[ptr HostTopic](topics), topics_count
  )

proc accessAccount(
    p: evmc_host_context, address: var evmc_address
): evmc_access_status {.cdecl.} =
...
proc setTransientStorage(
  ...
  toHost(p).setTransientStorage(
    address.fromEvmc, key.flip256.fromEvmc, value.flip256.fromEvmc
  )
...
proc evmcExecComputation*(host: TransactionHost): EvmcResult =
  ...
  result = vm.execute(
    vm,
    hostInterface.unsafeAddr,
    hostContext,
    evmc_revision(host.vmState.fork.ord),
    host.msg,
    if host.code.len > 0:
      host.code.bytes[0].unsafeAddr
    else:
      nil,
