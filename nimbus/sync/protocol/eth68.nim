...
const
  ...
  trEthRecvReceived* = "<< " & prettyEthProtoName & " Received "
  trEthRecvReceivedBlockHeaders* = trEthRecvReceived & "BlockHeaders (0x04)"
  trEthRecvReceivedBlockBodies* = trEthRecvReceived & "BlockBodies (0x06)"

  trEthRecvProtocolViolation* = "<< " & prettyEthProtoName & " Protocol violation, "
  trEthRecvError* = "<< " & prettyEthProtoName & " Error "
  trEthRecvTimeoutWaiting* = "<< " & prettyEthProtoName & " Timeout waiting "
  trEthRecvDiscarding* = "<< " & prettyEthProtoName & " Discarding "

  trEthSendSending* = ">> " & prettyEthProtoName & " Sending "
  trEthSendSendingGetBlockHeaders* = trEthSendSending & "GetBlockHeaders (0x03)"
  trEthSendSendingGetBlockBodies* = trEthSendSending & "GetBlockBodies (0x05)"

  trEthSendReplying* = ">> " & prettyEthProtoName & " Replying "

  trEthSendDelaying* = ">> " & prettyEthProtoName & " Delaying "

  trEthRecvNewBlock* = "<< " & prettyEthProtoName & " Received NewBlock"
  trEthRecvNewBlockHashes* = "<< " & prettyEthProtoName & " Received NewBlockHashes"
  trEthSendNewBlock* = ">> " & prettyEthProtoName & " Sending NewBlock"
  trEthSendNewBlockHashes* = ">> " & prettyEthProtoName & " Sending NewBlockHashes"
...
p2pProtocol eth68(
  ...
  onPeerConnected do(peer: Peer):
    ...
    trace trEthSendSending & "Status (0x00)",
      peer,
    ...
    if m.networkId != network.networkId:
      trace "Peer for a different network (networkId)",
        peer, expectNetworkId = network.networkId, gotNetworkId = m.networkId
  ...
  handshake:
    ...
    proc status(
      ...
      trace trEthRecvReceived & "Status (0x00)",
        peer,
        networkId,
        totalDifficulty,
        bestHash = short(bestHash),
        genesisHash = short(genesisHash),
        forkHash = forkId.forkHash.toHex,
        forkNext = forkId.forkNext

  # User message 0x01: NewBlockHashes.
  proc newBlockHashes(peer: Peer, hashes: openArray[NewBlockHashesAnnounce]) =
    when trEthTraceGossipOk:
      trace trEthRecvReceived & "NewBlockHashes (0x01)", peer, hashes = hashes.len
  ...
  proc transactions(peer: Peer, transactions: openArray[Transaction]) =
    when trEthTraceGossipOk:
      trace trEthRecvReceived & "Transactions (0x02)",
  ...
  requestResponse:
    ...
    proc getBlockHeaders(peer: Peer, request: BlocksRequest) =
      when trEthTracePacketsOk:
        trace trEthRecvReceived & "GetBlockHeaders (0x03)",
      ...
      if headers.get.len > 0:
        trace trEthSendReplying & "with BlockHeaders (0x04)",
          peer, sent = headers.get.len, requested = request.maxResults
      else:
        trace trEthSendReplying & "EMPTY BlockHeaders (0x04)",
  ...
  requestResponse:
    ...
    proc getBlockBodies(peer: Peer, hashes: openArray[Hash256]) =
      ...
      if bodies.get.len > 0:
        trace trEthSendReplying & "with BlockBodies (0x06)",
          peer, sent = bodies.get.len, requested = hashes.len
      else:
        trace trEthSendReplying & "EMPTY BlockBodies (0x06)",
  ...
  proc newBlock(peer: Peer, blk: EthBlock, totalDifficulty: DifficultyInt) =
    ...
    when trEthTraceGossipOk:
      trace trEthRecvReceived & "NewBlock (0x07)",
        peer,
        totalDifficulty,
  ...
  proc newPooledTransactionHashes(
      peer: Peer, txTypes: Blob, txSizes: openArray[int], txHashes: openArray[Hash256]
  ) =
    when trEthTraceGossipOk:
      trace trEthRecvReceived & "NewPooledTransactionHashes (0x08)",
        peer, txTypes = txTypes.toHex, txSizes, hashes = txHashes.len
  ...
  requestResponse:
    ...
    proc getPooledTransactions(peer: Peer, txHashes: openArray[Hash256]) =
      trace trEthRecvReceived & "GetPooledTransactions (0x09)",
      ...
      if txs.get.len > 0:
        trace trEthSendReplying & "with PooledTransactions (0x0a)",
          peer, sent = txs.get.len, requested = txHashes.len
      else:
        trace trEthSendReplying & "EMPTY PooledTransactions (0x0a)",
          peer, sent = 0, requested = txHashes.len

      await response.send(txs.get)

    # User message 0x0a: PooledTransactions.
    proc pooledTransactions(peer: Peer, transactions: openArray[PooledTransaction])
  ...
  requestResponse:
    ...
    proc getReceipts(peer: Peer, hashes: openArray[Hash256]) =
      ...
      if rec.get.len > 0:
        trace trEthSendReplying & "with Receipts (0x10)",
          peer, sent = rec.get.len, requested = hashes.len
      else:
        trace trEthSendReplying & "EMPTY Receipts (0x10)",
          peer, sent = 0, requested = hashes.len
