...
type BeaconEngineRef* = ref object
  txPool: TxPoolRef
  merge: MergeTracker
  queue: PayloadQueue
  chain: ChainRef
...
proc setInvalidAncestor(ben: BeaconEngineRef, invalid, origin: common.BlockHeader) =
...
proc new*(
    _: type BeaconEngineRef, txPool: TxPoolRef, chain: ChainRef
): BeaconEngineRef =
  ...
  txPool.com.notifyBadBlock = proc(
...
proc put*(ben: BeaconEngineRef, hash: common.Hash256, header: common.BlockHeader) =
...
proc put*(
  ...
  ben.queue.put(id, blockValue, payload, blobsBundle = Opt.none(BlobsBundleV1))
...
proc get*(
    ben: BeaconEngineRef, hash: common.Hash256, header: var common.BlockHeader
): bool =
...
proc generatePayload*(
    ben: BeaconEngineRef, attrs: PayloadAttributes
  ...
  wrapException:
    ...
    if bundle.blobsBundle.isSome:
      template blobData(): untyped =
        bundle.blobsBundle.get
    ...
    ok ExecutionPayloadAndBlobsBundle(
      executionPayload: executionPayload(bundle.blk), blobsBundle: blobsBundle
    )

proc setInvalidAncestor*(
    ben: BeaconEngineRef, header: common.BlockHeader, blockHash: common.Hash256
) =
...
proc checkInvalidAncestor*(
    ben: BeaconEngineRef, check, head: common.Hash256
): Opt[PayloadStatusV1] =
  # If the hash to check is unknown, return valid
  ben.invalidTipsets.withValue(check, invalid):
...
proc delayPayloadImport*(
    ben: BeaconEngineRef, header: common.BlockHeader
): PayloadStatusV1 =
