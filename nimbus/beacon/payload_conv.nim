...
import ./web3_eth_conv, web3/execution_types, ../utils/utils, eth/common

# ------------------------------------------------------------------------------
# Private helpers
# ------------------------------------------------------------------------------

func wdRoot(list: openArray[WithdrawalV1]): common.Hash256 {.gcsafe, raises: [].} =
  {.noSideEffect.}:
    calcWithdrawalsRoot(ethWithdrawals list)

func wdRoot(x: Opt[seq[WithdrawalV1]]): Opt[common.Hash256] {.gcsafe, raises: [].} =
  {.noSideEffect.}:
    if x.isNone:
      Opt.none(common.Hash256)
    else:
      Opt.some(wdRoot x.get)

func txRoot(list: openArray[Web3Tx]): common.Hash256 {.gcsafe, raises: [RlpError].} =
  {.noSideEffect.}:
...
func blockHeader*(
    p: ExecutionPayload, beaconRoot: Opt[common.Hash256]
...
func blockBody*(p: ExecutionPayload): common.BlockBody {.gcsafe, raises: [RlpError].} =
  common.BlockBody(
    uncles: @[],
