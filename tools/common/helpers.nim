...
proc createForkTransitionTable(transitionFork: HardFork, b: Option[BlockNumber], t: Option[EthTime], ttd: Option[DifficultyInt]): ForkTransitionTable =
  ...
  result.shanghaiTransitionThreshold.blockNumber = blockNumberToUse(HardFork.Shanghai)
  result.shanghaiTransitionThreshold.time = timeToUse(HardFork.Shanghai)
  
  for f in firstPurelyTimeBasedFork .. high(HardFork):
...
func getChainConfig*(network: string, c: ChainConfig) =
  ...
  of $TestFork.Shanghai:
    c.assignNumber(HardFork.Shanghai, BlockNumberZero)
