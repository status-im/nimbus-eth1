...
proc init*(
  ...
  let
    ...
    genesisValidatorsRoot = genesisHeader.genesis_validators_root
    clock = BeaconClock.init(cfg.timeParams, genesisHeader.genesis_time).valueOr:
      return err("Invalid genesis time in network metadata")
...
proc buildProof*(b: BeaconProofBuilder, timestamp: uint64): Result[Proof, string] =
  ...
  let (afterGenesis, slot) = b.clock.toSlot(times.fromUnix(timestamp.int64))
  if not afterGenesis:
    return err("EL block timestamp " & $timestamp & " is before beacon genesis")
