...
proc verifyResult(ctx: var StateContext, vmState: BaseVMState) =
  ...
  if obtainedHash != ctx.expectedHash:
    ctx.error =
      "post state root mismatch: got $1, want $2" %
  ...
  if actualLogsHash != ctx.expectedLogs:
    ctx.error =
      "post state log hash mismatch: got $1, want $2" %
...
when false:
  ...
  proc dumpState(vmState: BaseVMState): StateDump =
    StateDump(
      root: vmState.readOnlyStateDB.rootHash, accounts: dumpAccounts(vmState.stateDB)
    )

proc writeRootHashToStderr(vmState: BaseVMState) =
  let stateRoot = %{"stateRoot": %(vmState.readOnlyStateDB.rootHash)}
...
proc runExecution(ctx: var StateContext, conf: StateConf, pre: JsonNode): StateResult =
  ...
  vmState.init(parent = ctx.parent, header = ctx.header, com = com, tracer = tracer)
...
proc toTracerFlags(conf: StateConf): set[TracerFlags] =
  result = {TracerFlags.DisableStateDiff}

  if conf.disableMemory:
    result.incl TracerFlags.DisableMemory
  if conf.disableStack:
    result.incl TracerFlags.DisableStack
  if conf.disableReturnData:
    result.incl TracerFlags.DisableReturnData
  if conf.disableStorage:
  ...
  if conf.fork.len > 0:
    ...
    if conf.index.isNone:
      ...
      if index > forkData.len or index < 0:
        stdout.writeLine(
          "selected index out of range(0-$1), requested $2" % [$forkData.len, $index]
        )
        return false
