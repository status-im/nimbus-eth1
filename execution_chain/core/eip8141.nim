...
proc attemptApproval(cpt: Computation, scope: int, resolvedTarget: addresses.Address): EvmResult[bool] =
  let
    tx = cpt.getTransaction()
    frameIndex = cpt.getCurrentFrameIndex()
    frame = cpt.getFrame(frameIndex)
    allowedScope = frame.flags.int and APPROVE_SCOPE_MASK
  ...
  if approvesPayment:
    ...
    if not (cpt.senderApproved or approvesExecution):
      return ok(false)
    let
      payerBalance = ledger.getBalance(resolvedTarget)
    # The payer cannot cover the transaction's maximum cost.
    if payerBalance < cpt.getMaxCost().u256:
      return ok(false)

  if approvesExecution:
    cpt.senderApproved(true)
  if approvesPayment:
    if not ledger.isAccountAlive(tx.sender):
      ? cpt.chargeFrameStateGas(CREATE_ACCOUNT_STATE_GAS, "attemptApproval")

    ledger.incNonce(tx.sender)
    ledger.subBalance(resolvedTarget, cpt.getMaxCost().u256)
    cpt.payer(resolvedTarget)
