...
import
  ...
  ../evm/[evm_errors, state],
  ../constants,
  ../transaction/call_types,
  ../evm/secp256r1verify

from ./eip8037 import CREATE_ACCOUNT_STATE_GAS

const
  MAX_FRAMES = 64

const
  APPROVE_PAYMENT_FLAG*   = 0x1
  APPROVE_EXECUTION_FLAG* = 0x2
  ATOMIC_BATCH_FLAG*      = 0x4
  APPROVE_SCOPE_MASK*     = APPROVE_PAYMENT_FLAG + APPROVE_EXECUTION_FLAG
...
const
  FRAME_ENTRY_POINT* = address"0x00000000000000000000000000000000000000aa"
...
proc computeSigHash*(tx: Transaction): Hash32 =
  let rlpBytes = encodeForSigning(tx, true)
  keccak256(rlpBytes)
...
func validateTxEip8141*(tx: Transaction, intrinsic: IntrinsicGas): Result[void, string] =
  ...
  for i, frame in tx.frames:
    ...
    if (frame.flags and APPROVE_EXECUTION_FLAG) != 0:
    ...
    if (frame.flags and ATOMIC_BATCH_FLAG) != 0:
    ...
    if ((frame.flags and ATOMIC_BATCH_FLAG) != 0) or
       (i > 0 and ((tx.frames[i - 1].flags and ATOMIC_BATCH_FLAG) != 0)):
      if (frame.flags and APPROVE_SCOPE_MASK) != 0:
...
func validateSignature*(
        ...
        sender: Address,
        sigHash: Hash32): Result[Address, string] =
  ...
  if signatureScheme == SCHEME_ARBITRARY:
    ...
    return ok(zeroAddress)

  err("unsupported signature scheme")

proc attemptApproval*(vmState: BaseVMState,
                     frame: TransactionFrame,
                     scope: uint,
                     resolvedTarget: addresses.Address): EvmResult[bool] =
  let
  ...
  if (scope == 0) or ((scope and not allowedScope) != 0):
    return ok(false)

  let
    approvesExecution = (scope and APPROVE_EXECUTION_FLAG) != 0
    approvesPayment = (scope and APPROVE_PAYMENT_FLAG) != 0
    ledger = vmState.ledger

  if approvesExecution:
    # Execution is already approved.
    if vmState.senderApproved:
      return ok(false)
    # Only the sender may approve execution on its own behalf.
    if resolvedTarget != tx.sender:
      return ok(false)

  if approvesPayment:
    # Payment is already approved.
    if vmState.payer.isSome:
      return ok(false)
    # Payment approval requires execution approval first.
    if not (vmState.senderApproved or approvesExecution):
      return ok(false)

    if vmState.balTrackerEnabled:
      vmState.balTracker.trackAddressAccess(resolvedTarget)
    let
      payerBalance = ledger.getBalance(resolvedTarget)
    # The payer cannot cover the transaction's maximum cost.
    if payerBalance < vmState.getMaxCost():
      return ok(false)

  if approvesExecution:
    vmState.senderApproved(true)
  if vmState.balTrackerEnabled:
    vmState.balTracker.trackAddressAccess(tx.sender)
  if approvesPayment:
    if not ledger.isAccountAlive(tx.sender):
      ? vmState.chargeFrameStateGas(CREATE_ACCOUNT_STATE_GAS, "attemptApproval")

    let maxCost = vmState.getMaxCost()
    if vmState.balTrackerEnabled:
      vmState.balTracker.trackIncNonceChange(tx.sender)
      vmState.balTracker.trackSubBalanceChange(resolvedTarget, maxCost)

    ledger.incNonce(tx.sender)
    ledger.subBalance(resolvedTarget, maxCost)
    vmState.payer(resolvedTarget)

  ok(true)

func resolveFrameTarget*(tx: ptr Transaction, frame: ptr TransactionFrame): Address =
