...
const
  ...
  ATOMIC_BATCH*       = 0x4
  APPROVE_SCOPE_MASK* = APPROVE_PAYMENT + APPROVE_EXECUTION
...
proc attemptApproval*(vmState: BaseVMState,
                     ...
                     scope: uint, resolvedTarget:
                     addresses.Address): EvmResult[bool] =
  let
  ...
  let
    approvesExecution = (scope and APPROVE_EXECUTION) != 0
    approvesPayment = (scope and APPROVE_PAYMENT) != 0
  ...
  if approvesPayment:
    ...
    if payerBalance < vmState.getMaxCost().u256:
      return ok(false)
  ...
  if approvesPayment:
    ...
    ledger.subBalance(resolvedTarget, vmState.getMaxCost().u256)
