...
import
  ...
  beacon_chain/[buildinfo, conf, beacon_node, nimbus_binary_common, process_state],
...
proc main() {.noinline, raises: [CatchableError].} =
  ...
  if isBN:
    ...
    var config = XNimbusConf.loadWithBanners(banner, copyright, [specBanner], true).valueOr:
