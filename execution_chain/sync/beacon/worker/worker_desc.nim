...
type
  ...
  SchedPeerHdl* =
    proc(buddy: BeaconBuddyRef; rank: PeerRanking):
      Future[Duration] {.async: (raises: []).}
        ## See `runPeer()` described in `sync_sched.nim`

  GetBlockHeadersHdl* =
    proc(buddy: BeaconBuddyRef; req: BlockHeadersRequest; bn: BlockNumber):
  ...
  ImportBlockHdl* =
    proc(buddy: BeaconBuddyRef; blk: EthBlock; effPeerID: Hash):
