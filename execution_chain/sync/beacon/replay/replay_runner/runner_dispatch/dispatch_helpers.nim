...
# ------------------------------------------------------------------------------
# Private debugging helper(s)
# ------------------------------------------------------------------------------

func noisy(count: int): bool =
  if count < 100:
    return (count mod 55) == 0
  if count < 1000:
    return (count mod 111) == 0
  return (count mod 1111) == 0
...
template waitForConditionImpl(
  ...
  block body:
    let n {.inject.} = run.instrNumber
    var count {.inject.} = 0
    while true:
      count.inc

      if run.stopRunner:
        chronicles.info info & ": runner stopped", n, count
      ...
      try:
        ...
        chronicles.info info & ": cancelled while waiting -- STOP", n
...
proc getPeer*(
  ...
  trace info & ": no peer", n=run.iNum, serial=instr.serial,
...
proc delPeer*(
  ...
  if run.peers.hasKey(buddy.peerID):
    ...
    if run.noisy: trace info & ": stale peer ignored", n=buddy.iNum,
...
proc delDaemon*(
  ...
  let run = daemon.run
  if run.daemon.isNil:
    if run.noisy: trace info & ": stale daemon ignored", n=run.instrNumber
  else:
...
proc waitForSyncedEnv*(
  ...
  if run.noisy: trace info & ": process to be synced", n=desc.iNum, serial,
    peer, peerID

  let rc = desc.run.waitForConditionImpl(info):
    if count.noisy:                             # for logging/debugging only
      trace info & ": polling for sync", n=desc.iNum, serial, peer, count
      desc.checkSyncerState(instr, info)        # for logging/debugging only

    desc.syncedEnvCondImpl(instr, info)

  if rc.isErr():
    # Shutdown?
    if run.noisy: trace info & ": process sync error", n=desc.iNum,
      serial, peer, peerID, name=rc.error.name, msg=rc.error.msg
    return err(rc.error)

  if run.noisy: trace info & ": process synced ok", n=desc.iNum, serial,
    peer, peerID
...
proc processFinished*(
  ...
  if desc.run.noisy: trace info & ": terminating", n=desc.iNum,
...
template whenProcessFinished*(
  ...
  block body:
    ...
    if run.noisy: trace info & ": wait for terminated", n=desc.iNum, serial,
    ...
    bodyRc = desc.run.waitForConditionImpl(info):
      if count.noisy:
        trace info & ": polling for sync", n=desc.iNum, serial,
          peer, peerID, count
        desc.checkSyncerState(instr, info)
    ...
    if run.noisy: trace info & ": finished", n=desc.iNum, serial,
      frameID=instr.frameID.idStr, peer, peerID
    # End body
  ...
  block:
    ...
    block body:
      ...
      bodyRc = desc.run.waitForConditionImpl(info): # FIXME, really needed?
        ...
        desc.syncedEnvCondImpl(instr, info)         # FIXME, really needed?
      ...
      if desc.run.noisy: trace info & ": sent data", n=desc.iNum, serial,
        peer, peerID, dataType

      # Wait for message to be swallowed
      bodyRc = desc.run.waitForConditionImpl(info):
        if count.noisy:
          trace info & ": polling for ackn", n=desc.iNum, serial,
            peer, peerID, dataType, count
      ...
      if run.noisy: trace info & ": done", n=desc.iNum, serial,
        peer, peerID, dataType
...
template withInstr*(
  ...
  block:
    ...
    if run.noisy: trace info & ": get data", n=desc.iNum, serial="n/a",
      frameID="n/a", peer, dataType

    # Reset flag and wait for staged data to disappear from stack
    let rc = run.waitForConditionImpl(info):
      if count.noisy: trace info & ": polling for data", n=desc.iNum,
        serial="n/a", frameID="n/a", peer, peerID, dataType, count

      not desc.message.isNil
    ...
    if rc.isOk():
      ...
      if run.noisy: trace info & ": got data", n=desc.iNum, serial=instr.serial,
