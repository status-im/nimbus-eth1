...
proc replayStart*(
  ...
  if ctx.handler.version in {0, ReplayBaseHandlersID}:
    ctx.pool.handlers = ReplayRef(
      ...
      runner:           ReplayRunnerRef.init(ctx, startNoisy, fakeImport),
      ...
      syncBlockHeaders: syncHeadersMuted,
      getBlockBodies:   fetchBodiesReplay,
      syncBlockBodies:  syncBodiesMuted,
      importBlock:      importBlockReplay,
      syncImportBlock:  syncBlockMuted)
...
proc replayStart*(ctx: BeaconCtxRef) =
  ...
  if ctx.handler.version == ReplayBaseHandlersID:
    ...
    ctx.replayStart(hdl.strm, hdl.startNoisy, hdl.fakeImport)
