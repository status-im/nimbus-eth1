...
type
  ...
  TraceBaseHandlersRef* = ref object of BeaconHandlersRef
    ## Extension for caching state so that the tracer start can be
    ## synchronised with, e.g. after the syncer has started
    strm*: Stream
    nSessions*: int

  TraceRef* = ref object of BeaconHandlersRef
    ## Overlay handlers extended by descriptor data
    ctx*: BeaconCtxRef                ## Parent context
    outStream*: Stream                ## Dump file for ethxx data packets
