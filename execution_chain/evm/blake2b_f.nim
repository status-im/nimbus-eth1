...
import nimcrypto/utils, ./blake2b_f_simd
...
proc blake2b_F_nim*(input: openArray[byte], output: var openArray[byte]): bool =
...
proc blake2b_F*(input: openArray[byte], output: var openArray[byte]): bool =
  when blake2bSimdAvailable:
    blake2b_F_simd(input, output)
  else:
    blake2b_F_nim(input, output)
