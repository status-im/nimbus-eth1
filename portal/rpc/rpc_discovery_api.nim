...
proc installDiscoveryApiHandlers*(rpcServer: RpcServer, d: discv5_protocol.Protocol) =
  ...
  rpcServer.rpc(EthJson):
    ...
    proc discv5_lookupEnr(
        nodeId: NodeId
    ): Record {.async: (raises: [ValueError, CancelledError]).} =
    ...
    proc discv5_ping(
        enr: Record
    ): PongResponse {.async: (raises: [ValueError, CancelledError]).} =
      let
    ...
    proc discv5_findNode(
        enr: Record, distances: seq[uint16]
    ): seq[Record] {.async: (raises: [ValueError, CancelledError]).} =
      let
        node = toNodeWithAddress(enr)
    ...
    proc discv5_recursiveFindNodes(
        nodeId: NodeId
