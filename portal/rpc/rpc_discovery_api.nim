...
proc installDiscoveryApiHandlers*(rpcServer: RpcServer, d: discv5_protocol.Protocol) =
  ...
  rpcServer.rpcContext(EthJson):
    rpc("discv5_nodeInfo") do() -> NodeInfo:
      return d.routingTable.getNodeInfo()

    rpc("discv5_updateNodeInfo") do(kvPairs: seq[(string, string)]) -> NodeInfo:
    ...
    rpc("discv5_routingTableInfo") do() -> RoutingTableInfo:
      return getRoutingTableInfo(d.routingTable)

    rpc("discv5_addEnr") do(enr: Record) -> bool:
    ...
    rpc("discv5_addEnrs") do(enrs: seq[Record]) -> bool:
    ...
    rpc("discv5_getEnr") do(nodeId: NodeId) -> Record:
    ...
    rpc("discv5_deleteEnr") do(nodeId: NodeId) -> bool:
    ...
    rpc("discv5_lookupEnr") do(nodeId: NodeId) -> Record:
    ...
    rpc("discv5_ping") do(enr: Record) -> PongResponse:
      let
    ...
    rpc("discv5_findNode") do(enr: Record, distances: seq[uint16]) -> seq[Record]:
      let
        node = toNodeWithAddress(enr)
    ...
    rpc("discv5_talkReq") do(enr: Record, protocol, payload: string) -> string:
      let
        node = toNodeWithAddress(enr)
    ...
    rpc("discv5_recursiveFindNodes") do(nodeId: NodeId) -> seq[Record]:
