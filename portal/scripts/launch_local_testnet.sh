...
print_help() {
  cat <<EOF
Usage: $(basename "$0") [OPTIONS] -- [NIMBUS_PORTAL_CLIENT OPTIONS]
E.g.: $(basename "$0") --nodes ${NUM_NODES} --data-dir "${DATA_DIR}" # defaults
  ...
  --run-tests                       when enabled run tests else use "htop" to see the nimbus_portal_client processes without doing any tests
...
BINARIES="nimbus_portal_client"
if [[ "${PORTAL_BRIDGE}" == "1" ]]; then
  BINARIES="${BINARIES} nimbus_portal_bridge"
fi
...
if [[ "$BG_JOBS" != "$NUM_JOBS" ]]; then
  echo "$(( NUM_JOBS - BG_JOBS )) nimbus_portal_client instance(s) exited early. Aborting."
  dump_logs
  exit 1
fi
