...
proc injectEngineFrontend*(server: JsonRpcServer, frontend: ExecutionApiFrontend) =
  server.getServer().rpc(EthJson):
    ...
    proc eth_getBlockReceipts(
        blockTag: BlockTag
    ): Opt[seq[ReceiptObject]] {.async: (raises: [ValueError, CancelledError]).} =
      unpackEngineResult(await frontend.eth_getBlockReceipts(blockTag))

    proc eth_getTransactionReceipt(
        txHash: Hash32
    ): ReceiptObject {.async: (raises: [ValueError, CancelledError]).} =
      unpackEngineResult(await frontend.eth_getTransactionReceipt(txHash))

    proc eth_getLogs(
        filterOptions: FilterOptions
    ): seq[LogObject] {.async: (raises: [ValueError, CancelledError]).} =
    ...
    proc eth_uninstallFilter(
        filterId: string
    ): bool {.async: (raises: [ValueError, CancelledError]).} =
    ...
    proc eth_maxPriorityFeePerGas(): Quantity {.
        async: (raises: [ValueError, CancelledError])
    .} =
    ...
    proc eth_sendRawTransaction(
        txBytes: seq[byte]
    ): Hash32 {.async: (raises: [ValueError, CancelledError]).} =
