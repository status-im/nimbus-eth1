...
proc getEthApiBackend*(pool: JsonRpcClientPool): EthApiBackend =
  let
    ...
    getBlockByHashProc = proc(
      ...
      try:
        ok(
          await pool.getClientFromPool().resolveClient().eth_getBlockByHash(
    ...
    getBlockByNumberProc = proc(
      ...
      try:
        ok(
          await pool.getClientFromPool().resolveClient().eth_getBlockByNumber(
    ...
    getTransactionByHashProc = proc(
      ...
      try:
        ok(
          await pool.getClientFromPool().resolveClient().eth_getTransactionByHash(
            txHash
          )
        )
      except CancelledError as e:
    ...
    getTransactionReceiptProc = proc(
      ...
      try:
        ok(
          await pool.getClientFromPool().resolveClient().eth_getTransactionReceipt(
            txHash
          )
        )
      except CancelledError as e:
